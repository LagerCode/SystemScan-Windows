...
#TODO: fixa win upd
def Update():
    # Updating repos and system. Removes unnecisary packages
    try:

        update_process = subprocess.run(['powershell.exe','/home/lager/PycharmProjects/Work_Req/venv/syscan_linux/Sh_Filer/upgr.sh'],
                                        shell=False,
                                        stdout=True)
...
UpdateLabel =  Label(root,
                         text="Update your distro and repos")
...
#UpdateLabel.grid(row=5, column=1, pady=10)
UpdateButton.place(x=180, y=380)

#Check_For_MalwareLabel.grid(row=6, column=1, pady=10)
Check_For_MalwareButton.place(x=180, y=430)
